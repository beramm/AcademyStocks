...
func fetchBeachName(beachID: Int,completion: @escaping (Result<Data, Error>) -> Void){
    var components = URLComponents(
        string:
    ...
    var body: some View {
        VStack {
            if isLoading {
                ...
                Text(responseTextData)
                    .padding()
                Text(responseTextBeach)
                    .padding()
            }
        }
        .onAppear {
    ...
    func loadName() {
        ...
        fetchBeachName(beachID: beachIdTarget) { result in
            DispatchQueue.main.async {
                ...
                case .success(let data):
                    do {
                        let beaches = try JSONDecoder().decode([BeachName].self, from: data)

                        if let beach = beaches.first {
                            responseTextBeach = "\(beach.name)\n\(beach.address)"
                        } else {
                            responseTextBeach = "No data"
                        }

                    } catch {
                        responseTextBeach = "JSON error: \(error.localizedDescription)"
                    }
